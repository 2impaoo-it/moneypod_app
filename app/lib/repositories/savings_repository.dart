...
class SavingsRepository {
  ...
  Future<List<SavingsGoal>> getSavingsGoals() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(errorData['error'] ?? 'Không thể tải danh sách mục tiêu');
      }
    } catch (e) {
  ...
  Future<Map<String, dynamic>> depositToGoal({
    ...
    try {
      print('🔵 [SavingsRepo] Nạp $amount vào goal $goalId từ wallet $walletId');
  ...
  Future<void> updateSavingsGoal({
    ...
    try {
      ...
      if (deadline != null) requestBody['deadline'] = _formatDateTimeForServer(deadline);
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final transactions = data
            ...
            .toList();
        print('✅ [SavingsRepo] Lấy thành công ${transactions.length} giao dịch');
        return transactions;
      } else {
        final errorData = jsonDecode(response.body);
        throw Exception(errorData['error'] ?? 'Không thể tải lịch sử giao dịch');
      }
    } catch (e) {
      print('❌ [SavingsRepo] Lỗi: $e');
      rethrow;
